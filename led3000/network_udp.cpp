...
int NetworkUdp::try_to_connect(void)
{
    ...
    if (setsockopt(m_socket, SOL_SOCKET, SO_REUSEPORT, &(reuse_enable), sizeof(reuse_enable)) < 0) {
        RedDebug::err("set reuse_enable %u error:%d", reuse_enable, errno);
        close(m_socket);
        m_socket = -1;
        return -4;
    }
    ...
    else if (0 == r)
    {
        ...
        if (setsockopt(m_socket, IPPROTO_IP, IP_MULTICAST_TTL, (char *)&ttl, sizeof(ttl)) != 0) {
            RedDebug::err("Failed set ttl to %d err:%d", ttl, errno);
            close(m_socket);
            m_socket = -1;
            return -5;
        }
    ...
    if (ret < 0) {
        ...
        RedDebug::log("set multi loop_enable to %u success", loop_enable);
    }
...
NetworkUdp::NetworkUdp(string dstip, uint16_t source_port, uint16_t dst_port, int socket_fd): m_sn(1), m_socket(socket_fd)
{
    ...
    if (m_socket > 0) {
        ...
        if (setsockopt(m_socket, SOL_SOCKET, SO_REUSEPORT, &(reuse_enable), sizeof(reuse_enable)) < 0) {
            ...
            RedDebug::log("set reuse_enable to %u success", reuse_enable);
        }
    ...
    ret  = setsockopt(m_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, &(req), sizeof(req));//IPPROTO_IP通过IP组播，IP_ADD_MEMBERSHIP -> 加入组播组
    if (ret < 0) {
        RedDebug::err("add multi to %s error:%d", dstip.c_str(), errno);
        close(m_socket);
        m_socket = -1;
        return;
    }
    ...
    ret  = setsockopt(m_socket, IPPROTO_IP, IP_MULTICAST_LOOP, &(loop_enable), sizeof(loop_enable));
    if (ret < 0) {
        RedDebug::log("set loop_enable %u error:%d", loop_enable, errno);
        close(m_socket);
        m_socket = -1;
        return;
    } else {
        RedDebug::log("set loop_enable to %u success", loop_enable);
    }
}
...
#define MAX_BUFFER_LEN    256
    ...
    if (-1 == ret) {
        RedDebug::err("Send msg to server %s failed:%d\n",
